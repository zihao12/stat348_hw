...
* Here, $X$'s columns are assumed to be orthorgonal (may not have unit L2 norm). So I will get $\hat{beta}$ on standardized $X$, get shrinkaged coefficients, then transform back to the un-standardized version. 

But the formula does not seem to work even for X with orthonormal columns. I think the problem is that the lambda means different things in my formula and in `glmnet`
```{r}
set.seed(123)
X = svd(data_sim$X_train - colMeans(data_sim$X_train))$u ## for testing
n = nrow(X); p = ncol(X)
b = rnorm(p)
y = X %*%b + rnorm(n, mean = 0, sd = 0.01)
# y = data_sim$y_train
# y = y - mean(y)

cv.ridge = cv.glmnet(X, y,alpha=0, nfolds = 10)
lam = 100 * cv.ridge$lambda.min
