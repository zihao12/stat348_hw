...
classifier_logistic <- function(data_train, data_test, cost = c(1,1)){
  ...
  return(list(err_test = err_test, err_train = err_train))
}
...
#### fit with K-NN
```{r}
classifier_knn <- function(data_train, data_test, k){
  yhat_test <- class::knn(data_train[, 2:257], data_test[,2:257], data_train[,1], k = k)
  err_test = mean( yhat_test != data_test$y)
  yhat_train <- class::knn(data_train[, 2:257], data_train[, 2:257], data_train[,1], k = k)
  err_train = mean( yhat_train != data_train$y)
  return(list(err_test = err_test, err_train = err_train))
}
...
err_train <- c(outcome_logistic$err_train)
err_test <- c(outcome_logistic$err_test)

for(i in 2:length(ks)){
  out <- classifier_knn(data_train23, data_test23, ks[i])
  err_train <- c(err_train, out$err_train)
  err_test <- c(err_test, out$err_test)
}
```
