...
knn_cv <- function(data_train, k, d = 10, cost = c(1,1)){
  ...
  for(i in 1:d){
    ...
    idx = start:end
    pred <- class::knn(data_train[-idx, 2:257], data_train[idx, 2:257], data_train[-idx,1], k = k, prob = TRUE)
    yhat <- knn_pred(pred, p0)
    loss = loss +  
      compute_loss(data_train$y[idx], yhat, cost) * length(yhat)
  }
  return(loss/N)
}
